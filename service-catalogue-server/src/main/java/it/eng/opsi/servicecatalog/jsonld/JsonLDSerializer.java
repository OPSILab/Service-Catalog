...
public class JsonLDSerializer extends BeanSerializer {
	...
	static {
		...
		contextItems.put("acr", "https://across-h2020.eu/ns/sml");
		contextItems.put("dct", "http://purl.org/dc/terms");

		// TODO: We should probably add some other common namespaces, such as foaf or
		// xsd
	}

	JsonLDSerializer(BeanSerializerBase src, String[] jsonldContextURIs, String[] cpsvContextURIs) {
		super(src);
		this.jsonldContextURIs = jsonldContextURIs;
		this.cpsvContextURIs = cpsvContextURIs;
	}
	...
	public void serializeWithType(Object bean, JsonGenerator gen, SerializerProvider provider, TypeSerializer typeSer)
		...
		if (currentRecursionDepth == 1) {
			...
			/*
			 * Write different @context depending if seralizing whole Service Model object
			 * or only hasInfo as CPSV jsonld model
			 */
			if (bean.getClass().toString().equals("class it.eng.opsi.servicecatalog.model.HasInfo")) // CPSV Context
				gen.writeArray(cpsvContextURIs, 0, cpsvContextURIs.length);
			else // General context
				gen.writeArray(jsonldContextURIs, 0, jsonldContextURIs.length);
//            gen.writeObjectField("@context", filteredContext);

			// gen.writeStringField("@context",
			// "https://jira.iais.fraunhofer.de/stash/projects/ICTSL/repos/ids-infomodel-commons/raw/jsonld-context/3.0.0/context.jsonld");
			// // only add @context on top level

		}
		WritableTypeId typeIdDef = _typeIdDef(typeSer, bean, JsonToken.START_OBJECT);
		String resolvedTypeId = typeIdDef.id != null ? typeIdDef.id.toString()
				: typeSer.getTypeIdResolver().idFromValue(bean);
	...
	/**
	 * We need to add the fields of DatPayload to the context manually (if
	 * DatPayload present) as RFC 7519 requires the exact field names specified
	 * below without any prefix for JWTs.
	 * 
	...
	private void addJwtFieldsToContext(Object bean, Map<String, String> context) {
		if (bean == null
				|| bean.getClass().getName()
						.equals("com.sun.org.apache.xerces.internal.jaxp.datatype.XMLGregorianCalendarImpl")
				|| bean.getClass().getName().equals("org.apache.jena.ext.xerces.jaxp.datatype.XMLGregorianCalendarImpl")
				|| bean.getClass() == BigInteger.class)
			return;
		if (bean.getClass().getSimpleName().contains("DatPayload")) {
			...
			Stream.of(bean.getClass().getDeclaredFields()).forEach(f -> {
				...
				if (f.isSynthetic() || f.getType().isPrimitive() || f.getType().isEnum()
						|| f.getType().toString().contains("java.") || f.getType().toString().contains("javax."))
					return;
	...
	private void filterContextWrtBean(Object bean, Map<String, String> filteredContext) {
		if (bean == null
				...
				|| bean.getClass().getName().equals("org.apache.jena.ext.xerces.jaxp.datatype.XMLGregorianCalendarImpl")
				|| bean.getClass() == BigInteger.class)
			return; // XMLGregorianCalendarImpl causes infinite recursion

		// Check if RdfResource or TypedLiteral is used. They contain a field called
		// "type" which can reference to any namespace
		// Therefore it is vital to also check the value of the type field for prefixes
		// that need to be included in the context
		if (bean.getClass().getSimpleName().equals("RdfResource")
				|| bean.getClass().getSimpleName().equals("TypedLiteral")) {
		...
		Stream.of(bean.getClass().getMethods()).forEach(m -> {
			// run though all properties and check annotations. These annotations should
			...
			if (m.getReturnType().isEnum()) {
				...
				if (Arrays.stream(m.getReturnType().getEnumConstants())
						.anyMatch(constant -> constant.toString().startsWith(contextItems.get("idsc")))) {
		...
		for (Field f : getAllFields(new HashSet<>(), bean.getClass())) {
			...
			if (Collection.class.isAssignableFrom(f.getType())) {
				try {
					if (f.getType().getName().startsWith("java.") && !f.getType().getName().startsWith("java.util"))
						continue;
			...
			if (f.getType().isPrimitive() || f.getType().isEnum() || f.getType().isArray()
					|| f.getType().getName().contains("java.") || f.getType().getName().contains("javax."))
				continue;
			...
			try {
				...
				// logger.error("setting accessible failed"); //We can catch that here, as
				// IllegalReflectiveAccess cannot occur on our own packages
			}
	...
	/**
	 * This function retrieves a set of all available fields of a class, including
	 * inherited fields
	 * 
	 * @param fields Set to which discovered fields will be added. An empty HashSet
	 *               should do the trick
